...
function siguientePersonaje(){
    ...
    if(personajeActual==8){
        personajeActual = 1;
    }

    document.getElementById('personaje').src="p"+ personajeActual +".png"
    var sfxclic = new Audio ('clic.mp3');
    sfxclic.play()
}
...
function cargarEscenario(){
    ...
    for(i=0; i<marcador1; i++){
        document.querySelector('.vidas2').innerHTML +="<img src='calavera.png'>";
    }
    for(i=0; i<marcador2; i++){
        document.querySelector('.vidas1').innerHTML +="<img src='calavera.png'>";
    }
    if(marcador1>=3 || marcador2>=3){
        document.querySelector('.bg-juego').style.backgroundImage = "url(bg_personaje.png)";
        ...
        if(marcador1>=3){
            ...
            document.querySelector('#imgGanador').setAttribute('src', "p"+localStorage.getItem('personaje1')+".png");
            document.querySelector('.left').style.display="none";
            document.querySelector('.right').style.display="none";
        }else if(marcador2 >= 3){
            document.querySelector('#nombreGanador').innerHTML = localStorage.getItem('jugador2');
            document.querySelector('#imgGanador').setAttribute('src', "p"+localStorage.getItem('personaje2')+".png");
            document.querySelector('.left').style.display="none";
            document.querySelector('.right').style.display="none";
        }
    }else{
        ...
        document.querySelector('.bg-juego').style.backgroundImage = "url(bg"+bg+".png)";
    }
    

    //cargar personajes
    document.querySelector('.p1').style.backgroundImage = "url(p"+localStorage.getItem('personaje1')+".png)"
    document.querySelector('.p2').style.backgroundImage = "url(p"+localStorage.getItem('personaje2')+".png)"
